...
VERSIONE_PA = '1.2'
FORMATO_TRASMISSIONE_PA = 'FPA12'
FORMATO_TRASMISSIONE_PR = 'FPR12'
...
class WizardExportFatturapa(orm.TransientModel):
    ...
    def _setDatiAnagraficiCessionario(
        ...
        fatturapa.FatturaElettronicaHeader.CessionarioCommittente.DatiAnagrafici.Anagrafica = AnagraficaType(Denominazione=(partner.parent_id and partner.parent_id.name[0:80]) or partner.name[0:80])
    ...
    def setDatiGeneraliDocumento(self, cr, uid, invoice, body, context=None):
        ...
        for line in invoice.invoice_line:
            for tax in line.invoice_line_tax_id:
                ...
                elif tax.social_security and tax.social_security_type and not tax.social_security_type == 'TC07':
                ...
                elif tax.social_security and tax.social_security_type and tax.social_security_type == 'TC07':
                    # ENASARCO, values are in DettaglioLinee
                    pass
        ...
        if stamp_amount:
            ...
            invoice.write({'virtual_stamp': True, 'stamp_amount': 2})
        ...
        for line in inv.invoice_line:
            ...
            if sale_order:
                if sale_order not in okeys:
                    sale_order_ids = sale_order_obj.search(cr, uid, [('name', '=', sale_order)], context=context)
                    if sale_order_ids:
                        sale = sale_order_obj.browse(cr, uid, sale_order_ids[0], context)
                        okey = "{0}_{1}".format(sale.date_order, sale.name)
                        sale_orders[okey] = {
                            'origin': sale.name,
        ...
        if not no_tree:
            ...
            for order in OrderedDict(sorted(sale_orders.items(), key=lambda t: t[0])).values():
                ...
                if order.get('cig', False) or order.get('cup', False) or inv.cig or inv.cup:
                    ...
                    documento = DatiDocumentiCorrelatiType()
                    documento.IdDocumento = self.u_normalize(order['customer_ref'] and order['customer_ref'][0:20] or order['origin'][0:20])
                    documento.Data = order['date']
                    if order.get('cup', False) or inv.cup:
                        documento.CodiceCUP = self.u_normalize(order['cup'] or inv.cup)
                    if order.get('cig', False) or inv.cig:
                        documento.CodiceCIG = self.u_normalize(order['cig'] or inv.cig)
                    eval("body.DatiGenerali." + doc_type + ".append(documento)")
            ...
            for line in ddt['lines']:
                ...
                for tax in line.invoice_line_tax_id:
                    if tax.withholding_tax:
                        ...
                        if tax.social_security_type == 'TC07':
                            enasarco = True
                            riferimento_numero = '{:.8f}'.format(abs(tax.amount * line.price_subtotal))
                        else:
                ...
                if enasarco:
                    DettaglioLinea.AltriDatiGestionali.append(AltriDatiGestionaliType(
                        TipoDato='CASSA-PREV',
            ...
            try:
                ...
                for inv in invoice_browse_ids:
                    if inv.fatturapa_attachment_out_id:
                        error = _("Invoice %s has E-invoice Export File already") % (inv.number)
                        _logger.error(error)
