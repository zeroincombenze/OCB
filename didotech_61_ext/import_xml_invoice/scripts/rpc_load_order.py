...
if __name__ == "__main__":
    ...
    companies = rpc.execute('res.company', 'search_read', [], ('invoice_import_path', ))
    dir_path = companies[0]['invoice_import_path']
    for file_name in os.listdir(dir_path):
        if file_name[-3:] == 'xml':
            file_path = os.path.join(dir_path, file_name)
            os.chmod(file_path, 0664)

    rpc.execute('account.invoice', 'load_orders', False)
