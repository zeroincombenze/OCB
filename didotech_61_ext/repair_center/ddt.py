...
class Parser(report_sxw.rml_parse):
    ...
    def _get_utente(self):
        res = self.pool['res.users'].browse(self.cr, self.uid, self.uid, self.context)
        return res
        ...
        if order_name:
            ...
            if len(order_ids) == 1:
                ...
                if order.client_order_ref:
                    order_date = datetime.strptime(order.date_confirm or order.date_order, DEFAULT_SERVER_DATE_FORMAT)
                    description.append('{client_order} del {customer_order_date}'.format(client_order=order.client_order_ref, customer_order_date=order_date.strftime("%d/%m/%Y")))
        return '\n'.join(description)
    ...
    def _get_description(self, order_name):
        ...
        if order_name and not self.pool['res.users'].browse(
            ...
            if len(order_ids) == 1:
                ...
                order_date = datetime.strptime(order.date_confirm or order.date_order, DEFAULT_SERVER_DATE_FORMAT)
                if order.client_order_ref:
                    description.append(u'Rif. Ns. Conferma Ordine {order} del {order_date}, Vs. Ordine {client_order}'.format(order=order.name, order_date=order_date.strftime("%d/%m/%Y"), client_order=order.client_order_ref))
                else:
                    description.append(u'Rif. Ns. Conferma Ordine {order} del {order_date}'.format(order=order.name, order_date=order_date.strftime("%d/%m/%Y")))

        return ' / '.join(description)
    
    def _get_repair_order(self, stock_picking_id):
        repair_order_obj = self.pool['repair.order']
        repair_order_ids = repair_order_obj.search(self.cr, self.uid, [('out_picking_id', '=', stock_picking_id)], context=self.context)

        if repair_order_ids:
            return repair_order_obj.browse(self.cr, self.uid, repair_order_ids[0], self.context)
        else:
            return False
    ...
    def _raggruppa(self, righe_fattura):
        ...
        for riga in righe_fattura:
            if not (riga.origin in indice_movimenti and riga.origin in indice_movimenti[riga.origin]):
                if riga.origin:
                    # print 'Riga Buona'
    ...
    def _raggruppaddt(self, righe_ddt):
        ...
        for riga in righe_ddt:
            if not riga.origin in indice_movimenti:
            #     print riga.origin
            # else:
    ...
    def _indirizzo(self, partner):
        ...
        return self.pool['res.partner.address'].browse(self.cr, self.uid, address['invoice'] or address['default'], self.context)
